'''
To implement LBT framework on BERT model, majority of the algorithm is based on
...
from metrics.tasks_compute_metrics import compute_metrics
...
def train(args, train_dataset, model, tokenizer):
    ...
    print("The warmup_steps is: ",args.warmup_steps)
    ...
    logger.info("  Instantaneous batch size per {} = {}".format(args.device,args.per_gpu_train_batch_size))
    ...
    for current_epoch in range(int(args.num_train_epochs)):
        ...
        pbar = tqdm(train_dataloader,total=len(train_dataloader), desc='Training',ncols=120)
        for step, batch in enumerate(pbar):
            ...
            pbar.set_postfix(step=global_step,current_batch=current_epoch,loss='{0:1.5f}'.format(loss.item()))
            # logger.info("Current loss is {} in step {}/{}".format(loss,current_epoch,args.num_train_epochs))
            tr_loss += loss.item()
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1:  # Only evaluate when single GPU otherwise metrics may not average well
                        eval_scores = evaluate(args, model, tokenizer)
                        current_epoch_acc = eval_scores['acc']
                        if current_epoch_acc > all_best_eval_score:
                            all_best_eval_score = current_epoch_acc
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        pbar = tqdm(eval_dataloader,total=len(eval_dataloader), desc="Evaluating",ncols=120)
        for step, batch in enumerate(pbar):
            model.eval()
            batch = tuple(t.to(args.device) for t in batch)
            with torch.no_grad():
